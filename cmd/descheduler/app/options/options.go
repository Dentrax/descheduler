...
func (rs *DeschedulerServer) AddFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&rs.Tracing.OtelCollector, "otel-endpoint", "", "Set this flag to the OpenTelemetry Collector Service Address")
	fs.StringVar(&rs.Tracing.OtelTransportCert, "otel-transport-ca-cert", "", "Path of the CA Cert that can be used to generate the client Certificate for establishing secure connection to the OTEL in gRPC mode")
	fs.StringVar(&rs.Tracing.OtelTraceName, "otel-trace-name", tracing.DefaultServiceName, "OTEL Trace name to be used with the resources")
	fs.StringVar(&rs.Tracing.OtelTraceNamespace, "otel-trace-namespace", "", "OTEL Trace namespace to be used with the resources")
